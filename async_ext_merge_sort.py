...
async def merge(left_partition, right_partition, output_file_name):
    ...
    while left_remaining_count > 0 and right_remaining_count > 0:
        ...
        if left_pos == len(left_buffer):
            left_buffer = await read_element(left_file, left_remaining_count)
            left_pos = 0

        if right_pos == len(right_buffer):
            right_buffer = await read_element(right_file, right_remaining_count)
            right_pos = 0
        ...
        if len(content_buffer) == BUFFER_SIZE:
            await write_elements(output_file, content_buffer)
            content_buffer = []
    ...
    if len(content_buffer) > 0:
        await (output_file, content_buffer)
        content_buffer = []
...
async def merge_sort(start, end, files_list):
    ...
    if start == end:
        ...
        output_file_name = await get_output_file_name(start, end, len(files_list))
        ...
        sort_left_portion_task = asyncio.create_task(merge_sort(start, mid, files_list))
        sort_right_portion_task = asyncio.create_task(merge_sort(mid+1, end, files_list))

        left_partition = await sort_left_portion_task
        right_partition = await sort_right_portion_task

        combined_partition = await merge(left_partition, right_partition, output_file_name)

        remove(left_partition[0])
        remove(right_partition[0])

        return combined_partition


files_list = ['./input/'+file_name for file_name in listdir('./input') ]
asyncio.run(merge_sort(0, len(files_list)-1, files_list))
